  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
          ...
          <div className="space-y-2.5">
            <div className="flex items-center gap-3">
              ...
              <div className="space-y-1.5 flex-1">
                <div className="h-2.5 w-1/3 rounded-full bg-zinc-800/60 animate-pulse" />
                <div className="h-2 w-1/5 rounded-full bg-zinc-800/40 animate-pulse" />
              </div>
            </div>
            <div className="h-3.5 w-3/4 rounded-full bg-zinc-800/50 animate-pulse" />
            <div className="space-y-1.5">
              <div className="h-2.5 w-full rounded-full bg-zinc-800/40 animate-pulse" />
              <div className="h-2.5 w-2/3 rounded-full bg-zinc-800/40 animate-pulse" />
            </div>
            <div className="flex items-center justify-between pt-2 border-t border-zinc-800/30">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="h-2.5 w-8 rounded-full bg-zinc-800/40 animate-pulse" />
              ))}
            </div>
          </div>
        )}
        {variant === "profile-row" && (
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 rounded-full bg-zinc-800/60 animate-pulse shrink-0" />
            <div className="space-y-1.5 flex-1">
              <div className="h-2.5 w-1/3 rounded-full bg-zinc-800/60 animate-pulse" />
              <div className="h-2 w-1/5 rounded-full bg-zinc-800/40 animate-pulse" />
            </div>
        ...
        {variant === "message-sent" && (
          <div className="flex flex-col items-end gap-1.5">
            <div className="rounded-2xl rounded-tr-none bg-zinc-800/60 px-3 py-2 w-4/5">
              <div className="h-2 w-full rounded-full bg-zinc-700/60 animate-pulse" />
              <div className="h-2 w-2/3 rounded-full bg-zinc-700/40 animate-pulse mt-1.5" />
            </div>
          </div>
        )}
        {variant === "message-received" && (
          <div className="flex items-start gap-2.5">
            ...
            <div className="rounded-2xl rounded-tl-none bg-zinc-900/60 px-3 py-2 w-4/5">
              <div className="h-2 w-full rounded-full bg-zinc-800/40 animate-pulse" />
              <div className="h-2 w-1/2 rounded-full bg-zinc-800/30 animate-pulse mt-1.5" />
            </div>
          </div>
        )}
      </div>
