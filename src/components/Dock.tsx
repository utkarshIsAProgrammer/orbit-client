        ...
        >
          {item.specialPic && user?.profilePic?.url ? (
            <img loading="lazy"
              ...
              alt={user.fullName}
              className={`h-6 w-6 sm:h-8 sm:w-8 rounded-lg sm:rounded-xl object-cover border ${isActive ? "border-white" : "border-zinc-700"} shadow-sm`}
            />
          ) : (
            <Icon className={`h-4.5 w-4.5 sm:h-6 sm:w-6 ${isActive ? "text-black dark:text-white" : "text-zinc-400 dark:text-zinc-450 group-hover:text-black dark:group-hover:text-white"}`} />
          )}
